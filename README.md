...
Edit `./bedrock/.env.example` to your needs. During the `composer create-project` command described below, an `./bedrock/.env` will be created.
...
You can, but I recommend to use Composer for this only. But to enable this edit `./bedrock/config/environments/development.php` (for example to use it in Dev)
...
- Fixed a misstake so instead of `./srbedrockc/.env-example`, it should be `./bedrock/.env.example`.
